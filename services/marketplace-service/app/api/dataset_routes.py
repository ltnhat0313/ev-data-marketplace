...
# SỬA: Import từ deps thay vì user_routes (vì user_routes đã chuyển sang Auth Service)
...
def upload_dataset(
    ...
    # --- LOGIC MỚI: KIỂM TRA QUYỀN PROVIDER ---
    if current_user.role not in ["provider", "admin"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, 
            detail="Bạn không có quyền tải lên. Chỉ Data Provider hoặc Admin mới được phép."
        )
    # ------------------------------------------
    ...
    # Xử lý tên file an toàn hơn chút
...
def get_dataset_preview(
    ...
    try:
        if os.path.exists(d.file_path):
            ...
            preview_lines = ["File không tìm thấy trên server."]
    except Exception as e:
        preview_lines = [f"Lỗi đọc file: {str(e)}"]

...
async function loadSummary() {
  ...
  // Only run if latestList exists (Dashboard page)
  if (!latestList) return;

  try {
    // FIX: Add /api prefix
    const res = await fetch(`/api/datasets/stats/summary`);
    const data = await res.json();
    
    if (sumUsers) sumUsers.textContent = data.users ?? 0;
    if (sumDatasets) sumDatasets.textContent = data.datasets ?? 0;
    if (sumTxs) sumTxs.textContent = data.transactions ?? 0;
    ...
    (data.latest || []).forEach((d) => {
      ...
      // FIX: Use path parameter /ui/dataset/ID instead of query param
      title.href = `/ui/dataset/${d.id}`;
...
async function runSearch(pageOverride) {
  ...
  try {
    // FIX: Add /api prefix
    const res = await fetch(`/api/datasets/search?${params.toString()}`);
    const data = await res.json();
    ...
    if (tbody) {
      const rows = items.map(d => `
        <tr>
          <td><a href="/ui/dataset/${d.id}" style="text-decoration: none; color: #0b5fff; font-weight: 600;">${d.title}</a></td>
          ...
          <td><a href="/ui/dataset/${d.id}" class="btn btn-primary" style="text-decoration: none; padding: 6px 12px; font-size: 13px;">Xem chi tiết</a></td>
        </tr>
      `).join('');
...
// --- LOGIN FORM (ĐÃ CẬP NHẬT LOGIC ĐỒNG BỘ USER) ---
    ...
    fetch(`/api/auth/login`, {
      ...
      .then((d) => {
        if (d.access_token) {
          ...
          // --- BẮT ĐẦU LOGIC ĐỒNG BỘ ---
          // Gọi nhẹ một API Marketplace có bảo vệ để kích hoạt JIT Provisioning
          // Điều này đảm bảo user được tạo trong DB market trước khi chuyển trang
          showToast("Đang đồng bộ dữ liệu...", "info");
          
          fetch('/api/datasets/mine', {
          ...
          .catch((err) => {
              // Dù lỗi đồng bộ (hiếm gặp) vẫn cho vào dashboard để không chặn user
              console.warn("Sync warning:", err);
              showToast("Đăng nhập thành công (Sync pending)", "success");
              setTimeout(() => {
                window.location.href = "/ui/dashboard";
              }, 500);
          });
...
async function loadMyDatasets() {
  ...
  try {
    // FIX: Add /api prefix
    const res = await fetch(`/api/datasets/mine`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      .map(
        (d) =>
          `<div style="border:1px solid #d1d1d1;border-radius:8px;padding:10px;margin-bottom:8px;" data-id="${d.id}">
            <div><strong><a href="/ui/dataset/${d.id}" style="color:#0b5fff;text-decoration:none">${d.title}</a></strong></div>
    ...
    container.querySelectorAll(".btnEdit").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        const newPriceStr = prompt("Giá mới:", "0");
        ...
        // FIX: Add /api prefix
        const resp = await fetch(`/api/datasets/${id}`, { method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body: fd });
    ...
    container.querySelectorAll(".btnDelete").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        // FIX: Add /api prefix
        const resp = await fetch(`/api/datasets/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
        const out = await resp.json();
...
async function loadTwofaStatus() {
  ...
  try {
    // FIX: Add /api prefix
    const res = await fetch('/api/users/me/twofa/status', { headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json();
...
if (btnTwofaSetup) {
  btnTwofaSetup.addEventListener('click', async () => {
    ...
    // FIX: Add /api prefix
    const res = await fetch('/api/users/me/twofa/setup', { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
    ...
    if (qrBox) {
      ...
      try {
        // FIX: Add /api prefix
        const resp = await fetch(`/api/users/me/twofa/qr?ts=${Date.now()}`, { headers: { Authorization: `Bearer ${token}` } });
        if (resp.ok) {
...
if (btnTwofaEnable) {
  btnTwofaEnable.addEventListener('click', async () => {
    ...
    // FIX: Add /api prefix
    const res = await fetch(`/api/users/me/twofa/enable?code=${encodeURIComponent(code)}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
    const d = await res.json();
...
if (btnTwofaDisable) {
  btnTwofaDisable.addEventListener('click', async () => {
    ...
    const code = document.getElementById('twofaCode').value;
    // FIX: Add /api prefix
    const res = await fetch(`/api/users/me/twofa/disable?code=${encodeURIComponent(code)}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
    const d = await res.json();
